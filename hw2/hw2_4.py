...
class Portfolio(object):
	...
	def makeInvestment(self, number, what_investment, type = ""):
		if self.cash_value < (number * what_investment.price):
		...
		if type not in self.my_investments:
	...
	def sellInvestment(self, what_investment, number, bottom = 0.9, top = 1.2, x = 1, type = ""): # default selling values are for the mut. funds
		...
		selling_random = np.random.uniform(bottom * x, top * x, size=1)
 		(self.my_investments[type])[what_investment] = float("%.2f" %(((self.my_investments[type])[what_investment]) - number))
	...
	def buyStock(self, number, what_stock, type = "stocks"):
	...
	def sellStock(self, what_investment, number):
		self.sellInvestment(what_investment, number, bottom = 0.5, top = 1.5, x = dict_of_investments['stocks'][what_investment], type="stocks") # replaces def. values in sellInvestment with those appropriate for selling stocks
...
class Investments(object): # a parent class for stocks, mutual funds, bonds
	...
	def addToDict(self):
		if self.type not in dict_of_investments:
...
class Bond(Investments): # introducing bonds is this easy. This treats bonds w/ the same rules as mutual funds, but could be altered like for stocks
